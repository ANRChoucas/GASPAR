...
export const makeNewDefaultVictim = () => ({
  victim_id: `victim_${uuidv4()}`,
  health_status: {
    infos_general_physical_condition: '',
    infos_current_health: '',
    may_move: false,
...
function displayVictimInformations(card, infos) {
  card.setAttribute('data-id', infos.victim_id);
...
function fetchVictimInformations(card) {
  ...
  victim_informations.victim_id = card.getAttribute('data-id');
