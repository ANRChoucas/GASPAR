...
export default function ContextMenu() {
  ...
  this.showMenu = function showMenu(event, parent, items, onshow, onclose) {
    ...
    const hideMenu = (e) => {
      ...
      e.preventDefault();
      if (
        (e.target && e.target.className && e.target.className === 'separator')
        || (e.target && e.target.childNodes[0] && e.target.childNodes[0].className === 'separator')
      ) {
        // Don't do anything if the click happens on the separator element
        return;
      }
    ...
    setTimeout(() => {
      ...
      document.addEventListener('drag', hideMenu);
    }, 275);
  };
